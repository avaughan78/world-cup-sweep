...
export default function PrizeCard({ prize }: { prize: Prize }) {
  ...
  return (
    ...
    >
      <span className="text-xl leading-none">{prize.icon}</span>
      <p className="font-bold text-sm mt-2 leading-tight" style={{ color: 'var(--text-primary)' }}>
        {prize.name}
      </p>
      ...
      {hasLeader ? (
        ...
        <p className="text-xs" style={{ color: 'var(--text-muted)' }}>No leader yet</p>
      )}
    </div>
  );
}
